...
class Polyphony:
    ...
    def play_chord(self, notes, velocity, timing): #timing = number of ms between successive notes
        ...
        """
        self.midi.note_on(self.l.chord_channel, notes[0], velocity) 
        for i, n in enumerate(notes[1:], start=1):
            self.pending.append((n, velocity, time.ticks_ms()+timing*i))
        
    def stop_chord(self):
        self.midi.all_off(self.l.chord_channel)
        self.pending = []
        self.d.clear()
    ...
    def start_note(self, i):
        ...
        self.midi.note_on(self.l.melody_channel, self.scale[i]+transpose, 64)
    
    def stop_note(self, i):
        self.midi.note_off(self.l.melody_channel, self.scale[i]+self.melody_keys_transpose[i], 64)

    def stop_all_notes(self):
        self.midi.all_off(self.l.melody_channel)
    ...
    def loop(self):
        ...
        if len(self.pending):
            ...
            if next <= time.ticks_ms():
                ...
                self.midi.note_on(self.l.chord_channel, n[0], n[1])
