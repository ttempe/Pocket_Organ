...
# * add a logo for the battery level
...
class Display:
    def __init__(self):
        ...
        self.batt = label.Label(font_small, text="", x=57, y=5)
        self.tips_zone = label.Label(font_small, text="", x=0, y=45)
        self.tips_zone.anchor_point=(0,0)

        for i in [self.chord_name, self.latency, self.batt, self.tips_zone]:
    ...
    def disp_batt(self, lvl):
        self.batt.text = f"{int(lvl)}%" if lvl is not None else "CHRG"
        self.disp.refresh()
    ...
    def disp_slider(self, val, text):
        ...
        self.slider_zone.append(Rect(0, 0, val, 33, fill=WHITE, outline=WHITE)) #Todo: update lib and try self.slider_zone[1].wdith=val
        self.disp.refresh()
